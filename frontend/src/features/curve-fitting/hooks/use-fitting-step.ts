...
export function useFittingStep() {
  ...
  const fittedSeries = useMemo<CurveSeries[]>(() => {
    ...
    state.selectedModels.forEach((model) => {
      ...
      const displayEnd = Math.max(measuredEnd, end + Math.max(end - start, 0.05));
      const points = Array.from({ length: 301 }, (_, index) => {
        const strain = start + ((displayEnd - start) * index) / 300;
        return {
          strain,
      ...
      series.push({
        name: `${HARDENING_MODEL_LABELS[model]} 接続補正後`,
        points,
